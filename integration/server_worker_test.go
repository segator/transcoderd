...
// TestServerWorkerIntegration es un test de integración whitebox que:
// 1. Levanta un servidor con un scheduler
// 2. Simula un worker que pide un job
// 3. El worker "encodea" el archivo (simulado)
// 4. Verifica que el servidor registre el job como completado
func TestServerWorkerIntegration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	// Configurar el repositorio en memoria (mock)
	...
	sched, err := scheduler.NewScheduler(config, repo)
	if err != nil {
		t.Fatalf("Failed to create scheduler: %v", err)
	}
	...
	t.Run("Schedule Job", func(t *testing.T) {
		...
		result, err := sched.ScheduleJobRequests(ctx, jobRequest)
		if err != nil {
			t.Fatalf("Failed to schedule job: %v", err)
		}
	...
	t.Run("Request Job", func(t *testing.T) {
		...
		jobResponse, err := sched.RequestJob(ctx, workerName)
		if err != nil {
			t.Fatalf("Failed to request job: %v", err)
		}
		...
		// Verificar que el job está en estado Assigned
		job, err := repo.GetJob(ctx, jobResponse.Id.String())
		if err != nil {
			t.Fatalf("Failed to get job: %v", err)
		}
	...
	// Test 3: Worker reporta inicio del job
	t.Run("Worker Starts Job", func(t *testing.T) {
		jobs, _ := repo.GetJobsByStatus(ctx, model.JobNotification, model.AssignedNotificationStatus)
		if len(jobs) == 0 {
			t.Fatal("No assigned jobs found")
		}
		...
		err := sched.HandleWorkerEvent(ctx, envelope)
		if err != nil {
			t.Fatalf("Failed to handle start event: %v", err)
		}

		// Verificar estado
		job, _ = repo.GetJob(ctx, job.Id.String())
		status := job.Events.GetStatus()
		if status != model.StartedNotificationStatus {
			t.Errorf("Expected status %s, got %s", model.StartedNotificationStatus, status)
		}
	...
	// Test 4: Simular encoding (download, encode, upload)
	t.Run("Simulate Encoding Process", func(t *testing.T) {
		jobs, _ := repo.GetJobsByStatus(ctx, model.JobNotification, model.StartedNotificationStatus)
		if len(jobs) == 0 {
			t.Fatal("No started jobs found")
		}
	...
	t.Run("Complete Job", func(t *testing.T) {
		...
		// Verificar estado final
		job, err = repo.GetJob(ctx, job.Id.String())
		if err != nil {
			t.Fatalf("Failed to get final job: %v", err)
		}
	...
	// Test 6: Verificar historial de eventos
	t.Run("Verify Event History", func(t *testing.T) {
		jobs, _ := repo.GetJobsByStatus(ctx, model.JobNotification, model.CompletedNotificationStatus)
		if len(jobs) == 0 {
			t.Fatal("No completed jobs found")
		}
