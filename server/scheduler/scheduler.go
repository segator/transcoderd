	...
	filepath.Walk(filepath.Join(R.config.DownloadPath,parentJobRequest.SourcePath), func(pathFile string, f os.FileInfo, err error) error {
		...
		default:
			...
			if f.Size() <R.config.MinFileSize {
				jobRequestErrors= append(jobRequestErrors, fmt.Errorf("%w: %s File Size bigger than %d",ErrorFileSkipped,pathFile,R.config.MinFileSize))
			}
			extension := filepath.Ext(f.Name())[1:]
			if !helper.ValidExtension(extension) {
				jobRequestErrors= append(jobRequestErrors,fmt.Errorf("%w: %s Invalid Extension %s",ErrorFileSkipped,pathFile,extension))
			}

			relativePath, err := filepath.Rel(R.config.DownloadPath, filepath.FromSlash(pathFile))
			if err != nil {
				jobRequestErrors= append(jobRequestErrors,fmt.Errorf("%w: %v",ErrorFileSkipped,err))
			}
	...
	err = R.repo.WithTransaction(ctx,func(ctx context.Context,tx repository.Repository) error{
		...
		if video==nil{
			...
			if (jobRequest.ForceCompleted && status == model.CompletedNotificationStatus) ||
			   ...
			   (jobRequest.ForceAdded && status == model.AddedNotificationStatus) || (jobRequest.ForceAdded && status == model.ReAddedNotificationStatus)) {
